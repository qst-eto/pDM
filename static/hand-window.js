...
function handCardMarkup(item) {
  ...
  const shown = item.shown_to_opponent ? '<span class="shown-card-badge">相手に公開中</span>' : '';
  if (!item.face_up || !item.card) return `<button class="hand-card${selected}" data-uid="${handEscape(item.uid)}"><span class="card-back ${handState.backStyle === 'dummy' ? 'dummy-back' : ''}">DM</span>${note}${shown}</button>`;
  ...
  return `<button class="hand-card${selected}" data-uid="${handEscape(item.uid)}"><span class="card-face">${image}</span>${note}${shown}</button>`;
}
...
function handSignature(hand) {
  return JSON.stringify((hand || []).map((item) => [
    ...
    item.note || '',
    Boolean(item.shown_to_opponent),
  ]));
}
...
function renderHand() {
  ...
  const pinned = handState.hand.find((item) => handState.selected.has(item.uid));
  if (pinned) showHandPreview(pinned);
}
...
function showHandMenu(event, uid) {
  ...
  const selectedItems = handState.hand.filter((candidate) => handState.selected.has(candidate.uid));
  const allShown = selectedItems.length > 0 && selectedItems.every((candidate) => candidate.shown_to_opponent);
  menu.innerHTML = `<button data-hand-note>${item?.note ? 'メモを編集する' : 'メモをつける'}</button><button data-hand-stack>カードを重ねる ▶</button><button data-zone="mana">マナへ</button><button data-zone="graveyard">墓地へ</button><button data-zone="waiting">待機ゾーンへ</button><button data-zone="waiting" data-keep-face-down="true">相手に非公開で待機ゾーンへ</button><button data-zone="battle">バトルゾーンへ</button><button data-zone="shields">シールドゾーンへ</button><button data-zone="shields" data-position="face_up">表向きでシールドゾーンへ</button>${specialZoneMenuMarkup("hand-special-zones")}<button data-zone="deck" data-position="top">山札の一番上へ</button><button data-zone="deck" data-position="bottom">山札の一番下へ</button><div class="menu-separator"></div><button data-command="tap" data-value="true">タップする</button><button data-command="tap" data-value="false">アンタップする</button><button data-command="flip" data-value="false">裏向きにする</button>`;
  const showButton = document.createElement('button');
  showButton.dataset.handShow = '';
  showButton.dataset.value = String(!allShown);
  showButton.textContent = allShown ? '相手に見せるのをやめる' : '相手に見せる';
  menu.insertBefore(showButton, menu.querySelector('[data-hand-note]'));
  ...
  showButton.addEventListener('click', () => sendHandCommand({
    command: 'set_hand_card_visibility',
    card_ids: Array.from(handState.selected),
    value: showButton.dataset.value === 'true',
  }));
  menu.querySelector('[data-hand-note]').addEventListener('click', () => {
    const note = window.prompt('カードのメモ（空欄で削除・120文字まで）', item?.note || '');
    if (note !== null) sendHandCommand({ command: 'set_note', card_ids: Array.from(handState.selected), note });
  });
  menu.querySelectorAll('[data-zone]').forEach((button) => button.addEventListener('click', () => sendHandCommand({ command: 'move', card_ids: Array.from(handState.selected), zone: button.dataset.zone, position: button.dataset.position || 'append', target_player: 0, keep_face_down: button.dataset.keepFaceDown === 'true' })));
